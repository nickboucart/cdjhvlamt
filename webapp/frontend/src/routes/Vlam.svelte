<script>
	...
	import { getData, postData, getDataForQuery } from "../helpers/APIHelpers";

	let vlam;

	const queryResult = useQuery("getVlam" + $$props.id, () =>
		getDataForQuery(("/vlammeke/" + $$props.id)), {staleTime: 3*60*1000});
...
<p>
	...
	<button on:click={() => onClick($queryResult.data.thingName, "fire")}>vlam animatie</button>
	<button on:click={() => onClick($queryResult.data.thingName, "conjunction")}>andere animatie</button>
</p>
{/if}
	<!-- {#await getVlam()}
